...
export const useSalesProductStore = defineStore('sales-product', {
  state: () => ({
  ...
  actions: {
    ...
    async fetchReportSalesQtyList() {
      const { data }: any = await useAPIFetch('/IS_OU_REPORT_SALES_QTY_LIST_GET', {
        method: 'post',
        body: {
          ...this.filters,
