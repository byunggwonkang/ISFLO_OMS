...
export const useExchangeListStore = defineStore('exchange-list', {
  state: () => ({
  ...
  actions: {
    ...
    async fetchExchangeDetailInfoCSNote(exchangeId: string) {
      const { data }: any = await useAPIFetch('/IS_EU_EXCHANGE_DETAIL_INFO_NOTE_GET', {
        method: 'post',
        body: {
          USER_SID: encrypt(user.value?.USER_SID),
          FLT_EXCHANGE_SID: encrypt(exchangeId),
    ...
    async addExchangeOperationNote(formData: any) {
      const { data }: any = await useAPIFetch('/IS_EU_EXCHANGE_DETAIL_INFO_NOTE_SET', {
        method: 'post',
        body: {
          USER_SID: encrypt(user.value?.USER_SID),
          FLT_NOTE_TYPE: encrypt('O'),
