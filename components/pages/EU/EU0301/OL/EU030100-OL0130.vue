...
const { StationList, filters0030 } = storeToRefs(useOrderListStore())
...
onMounted(async () => {
  ...
  filters0030.value.FLT_STATUS = [ "90", "91", "92", "93", "94", "95", "96", "97", "98" ];
  filters0030.value.FLT_CHANNELS = StationList.value[0].STATION_CHANNEL_LIST.map((channel) => decrypt(channel.CHANNEL_CODE))
  filters0030.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters0030.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
  filters0030.value.FLT_INCLUDE_REQSUB = "1";
  filters0030.value.USER_SID = Station.value?.value
  filters0030.value.FLT_DATE_FROM = ''
  filters0030.value.FLT_DATE_TO = ''

  const { data } = await orderStatus.fetchOrderDetailList0030();
  orderDetailList.value = data
/*
  if (success) {
    filters0030.value.FLT_STATUS = [ "90", "91", "92", "93", "94", "95", "96", "97", "98" ];
    filters0030.value.FLT_CHANNELS = userGroupChannelList.value.map((channel) => channel.USER_CHANNEL_CODE)
    filters0030.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
    filters0030.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
    filters0030.value.FLT_INCLUDE_REQSUB = "1";
...
const onClickOrderDetailRefresh = async () => {
  ...
  filters0030.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters0030.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
  filters0030.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters0030.value.FLT_INCLUDE_REQSUB = "1";
  filters0030.value.USER_SID = Station.value?.value
  filters0030.value.FLT_DATE_FROM = ''
  filters0030.value.FLT_DATE_TO = ''
...
const orderDetailTableColumnsByOrderID = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU030100_GRD03001')
})

const orderDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU030100_GRD03002')
})

const tableId = computed(() => {
  return filters0030.value.FLT_VIEW_OPT === '0010' ? 'EU030100_GRD03001' : 'EU030100_GRD03002'
})
  ...
  <div class="space-y-3 pl-3">
    ...
    >
      <div class="grid grid-cols-5 gap-y-3 gap-x-12">       
        <FormGroup
          :label="getComponentData('EU030100_LBL03001', 'caption', 'Station ID')"
          label-id="EU030100_LBL03001"
          label-for="EU030100_CMB03001"
          horizontal
          label-width="w-1/3"
          input-width="w-2/3"
        >
          <FormSelect
            id="EU030100_CMB03001"
            v-model="Station"
    ...
    >
      <template #header>
        <div class="grid grid-cols-5 gap-y-3 gap-x-12"> 
          <FormGroup
            :label="getComponentData('EU030100_LBL03002', 'caption','Select Channel')"
            horizontal
            label-width="w-1/3"
            input-width="w-2/3"
            label-id="EU030100_LBL03002"
          >
            <FormSelect
              id="EU030100_CMB03002"
              v-model="channelList"
              multiple
          ...
          <FormGroup
            :label="getComponentData('EU030100_LBL03003', 'caption','View Type')"
            horizontal
            label-width="w-1/3"
            input-width="w-2/3"
            label-id="EU030100_LBL03003"
          >
            <FormSelect
              id="EU030100_CMB03003"
              v-model="viewType"
          ...
          <FormGroup
            :label="getComponentData('EU030100_LBL03004', 'caption','Split Order')"
            horizontal
            label-width="w-1/3"
            input-width="w-2/3"
            label-id="EU030100_LBL03004"
          >
            <FormSelect
              id="EU030100_CMB03003"
              v-model="orderType"
          ...
          <div class="flex">
            ...
            >
              {{ getComponentData ('EU030100_BTN03001', 'caption','Search') }}
            </Button>
          </div>
        </div>
      </template>
      <div class="pl-3 space-y-2">
        ...
        <Table
          v-if="tableId === 'EU030100_GRD03001'"
          id="EU030100_GRD03001"
          clickable
          checkable
        ...
        <Table
          v-if="tableId === 'EU030100_GRD03002'"
          id="EU030100_GRD03002"
          clickable
          checkable
          header-variant="secondary"
          class="overflow-x-auto border-b"
    ...
    >
      <Button
        ...
        :loading="loading"
        @click="updateOrderSubStatus('0181')"
      >
        {{ getComponentData('EU030100_BTN03002', 'caption','Reject Cancel') }}
      </Button>
      ...
      <Button
        ...
        :disabled="loading || orderDetailList?.length == 0"
        :loading="loading"
        @click="updateOrderSubStatus('0281')"
      >
        {{ getComponentData('EU030100_BTN03003', 'caption','Confirm Cancel') }}
      </Button>
    </div>
  </div>
</template>
