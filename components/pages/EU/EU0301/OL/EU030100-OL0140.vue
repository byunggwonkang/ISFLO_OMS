...
const { StationList, filters0040 } = storeToRefs(useOrderListStore())
...
onMounted(async () => {
  ...
  filters0040.value.FLT_ORDER_TYPES = [ "C" ];
  filters0040.value.FLT_CHANNELS = StationList.value[0].STATION_CHANNEL_LIST.map((channel) => decrypt(channel.CHANNEL_CODE))
  filters0040.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters0040.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
  filters0040.value.FLT_INCLUDE_REQSUB = "1";
  filters0040.value.USER_SID = Station.value?.value
  filters0040.value.FLT_DATE_FROM = ''
  filters0040.value.FLT_DATE_TO = ''
   ...
   /* channelList.value = userGroupChannelList.value.map((channel) => {
    ...
    const { data } = await orderStatus.fetchOrderDetailList0040();
    orderDetailList.value = data
 /* if (success) {
    filters0040.value.FLT_ORDER_TYPES = [ "C" ];
    filters0040.value.FLT_CHANNELS = userGroupChannelList.value.map((channel) => channel.USER_CHANNEL_CODE)
    filters0040.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
    filters0040.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
    filters0040.value.FLT_INCLUDE_REQSUB = "1";
...
const onClickOrderDetailRefresh = async () => {
  ...
  filters0040.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters0040.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
  filters0040.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters0040.value.FLT_INCLUDE_REQSUB = "1";
  filters0040.value.USER_SID = Station.value?.value
  filters0040.value.FLT_DATE_FROM = ''
  filters0040.value.FLT_DATE_TO = ''
...
const orderDetailTableColumnsByOrderID = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU030100_GRD04001')
})

const orderDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU030100_GRD04002')
})

const tableId = computed(() => {
  return filters0040.value.FLT_VIEW_OPT === '0010' ? 'EU030100_GRD04001' : 'EU030100_GRD04002'
})
  ...
  <div class="space-y-3 pl-3">
    ...
    >
      <div class="grid grid-cols-5 gap-y-3 gap-x-12">       
        <FormGroup
          :label="getComponentData('EU030100_LBL04001', 'caption', 'Station ID')"
          label-id="EU030100_LBL04001"
          label-for="EU030100_CMB04001"
          horizontal
          label-width="w-1/3"
          input-width="w-2/3"
        >
          <FormSelect
            id="EU030100_CMB04001"
            v-model="Station"
    ...
    >
      <template #header>
        <div class="grid grid-cols-5 gap-y-3 gap-x-12"> 
          <FormGroup
            :label="getComponentData('EU030100_LBL04002', 'caption','Select Channel')"
            horizontal
            label-id="EU030100_LBL04002"
            label-width="w-1/3"
            input-width="w-2/3"
          >
            <FormSelect
              id="EU030100_CMB04002"
              v-model="channelList"
              multiple
          ...
          <FormGroup
            :label="getComponentData('EU030100_LBL04003', 'caption','View Type')"
            horizontal
            label-id="OU030100_LBL04003"
            label-width="w-1/3"
            input-width="w-2/3"
          >
            <FormSelect
              id="EU030100_CMB04003"
              v-model="viewType"
          ...
          <FormGroup
            :label="getComponentData('EU030100_LBL04004', 'caption','Split Order')"
            horizontal
            label-id="EU030100_LBL04004"
            label-width="w-1/3"
            input-width="w-2/3"
          >
            <FormSelect
              id="EU030100_CMB04004"
              v-model="orderType"
          ...
          <div class="flex">
            ...
            >
              {{ getComponentData ('EU030100_BTN04001', 'caption','Search') }}
            </Button>
          </div>
        </div>
      </template>
      <div class="pl-3 space-y-2">
        <span v-if="getGridData('EU030100_GRD04001', 'display') === 'N'">
          {{ getGridData('EU030100_GRD04002', 'caption') }}
        </span>

        <Table
          v-if="tableId === 'EU030100_GRD04001'"
          id="EU030100_GRD04001"
          clickable
          checkable
        ...
        <Table
          v-if="tableId === 'EU030100_GRD04002'"
          id="EU030100_GRD04002"
          clickable
          checkable
          header-variant="secondary"
          class="overflow-x-auto border-b"
    ...
    >
      <Button
        ...
        :loading="loading"
        @click="updateOrderSubStatus('0173')"
      >
        {{ getComponentData('EU030100_BTN04002', 'caption','Reject Change') }}
      </Button>
      ...
      <Button
        ...
        :disabled="loading || orderDetailList?.length == 0"
        :loading="loading"
        @click="updateOrderSubStatus('0273')"
      >
        {{ getComponentData('EU030100_BTN04003', 'caption','Confirm Change') }}
      </Button>
    </div>
  </div>
</template>
