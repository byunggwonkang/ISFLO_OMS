...
const { userGroupChannelList, filters30 } = storeToRefs(returnList)
...
onMounted(async () => {
  ...
  if (success) {
    ...
    const { data } = await returnList.fetchReturnDetailList30()
...
const onClickReturnDetailRefresh = async () => {
  ...
  filters30.value.FLT_INBOUND_TYPE = ReturnInboundTypeData?.value.value ?? '';
...
const returnDetailTableColumnsByReturnId = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU040100_GRD03001')
})
...
<template>
  <div class="space-y-3">
    <div class="grid grid-cols-5 gap-3 items-center pl-3 pb-3 border-b">
      <FormGroup
        :label="getComponentData('EU040100_LBL03001', 'caption', 'Brand Channel')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB03001"
          v-model="channelList"
          multiple
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL03002', 'caption', 'Inbound Type')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB03002"
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL03003', 'caption','Return Reason')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB03003"
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL03004', 'caption','Key Word')"
        horizontal
        label-id="EU040100_LBL03004"
        label-width="w-1/3"
      >
        <FormTextInput
          id="EU040200_TXT03001"
      ...
      >
        {{ getComponentData ('EU040100_BTN03001', 'caption','SEARCH') }}
      </Button>
    </div>

    <div class="pl-3 space-y-2">
      <span>
        {{ getComponentData('EU040100_GRD03001', 'caption','Return Request Cancel list') }}
      </span>

      <Table
        id="EU040100_GRD03001"
        clickable
