...
const { userGroupChannelList, filters60 } = storeToRefs(returnList)
...
onMounted(async () => {
  ...
  if (success) {
    ...
    filters60.value.FLT_DATE_TYPE = dateType?.value.value ?? '';
    filters60.value.FLT_CHANNELS = userGroupChannelList.value.map((channel) => decrypt(channel.USER_CHANNEL_CODE))
    filters60.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
    filters60.value.FLT_RETURN_TYPES = ["C"];

    const { data } = await returnList.fetchReturnDetailList60()
...
const onClickReturnDetailRefresh = async () => {
  filters60.value.FLT_STATUS = returnStatusData?.value.map((status) => status.value) ?? []
  filters60.value.FLT_RETURN_TYPES = ["C"];
  filters60.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters60.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters60.value.FLT_DATE_TYPE = dateType?.value.value ?? '';
  filters60.value.FLT_REASON = returnReasonData?.value.map((reason) => reason.value) ?? []
  filters60.value.FLT_INBOUND_TYPE = ReturnInboundTypeData?.value.value ?? '';
...
const returnDetailTableColumnsByReturnId = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU040100_GRD06001')
})
...
<template>
  <div class="space-y-3">
    <div class="grid grid-cols-5 gap-3 items-center pl-3 pb-3 border-b">
      <FormGroup
        :label="getComponentData('EU040100_LBL06001', 'caption', 'Brand Channel')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB06001"
          v-model="channelList"
          multiple
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL06002', 'caption', 'Inbound Type')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB06002"
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL06003', 'caption','Return Reason')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB06003"
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL06004', 'caption','Key Word')"
        horizontal
        label-id="OU030100_LBL02001"
        label-width="w-1/3"
      >
        <FormTextInput
          id="EU040200_TXT06001"
          v-model="filters60.FLT_KEY_WORD"
        />
      </FormGroup>
      ...
      >
        {{ getComponentData ('EU040100_BTN06001', 'caption','SEARCH') }}
      </Button>
    </div>

    <div class="pl-3 space-y-2">
      <span>
        {{ getComponentData('EU040100_GRD06001', 'caption','Return Hold By ESM list') }}
      </span>

      <Table
        id="EU040100_GRD06001"
        clickable
