...
const { userGroupChannelList, filters50 } = storeToRefs(returnList)
...
onMounted(async () => {
  ...
  if (success) {
    ...
    filters50.value.FLT_DATE_TYPE = dateType?.value.value ?? '';
    ...
    filters50.value.FLT_RETURN_TYPES = ["C"];

    const { data } = await returnList.fetchReturnDetailList50()
...
const onClickReturnDetailRefresh = async () => {
  filters50.value.FLT_STATUS = returnStatusData?.value.map((status) => status.value) ?? []
  filters50.value.FLT_RETURN_TYPES = ["C"];
  filters50.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters50.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters50.value.FLT_DATE_TYPE = dateType?.value.value ?? '';
  filters50.value.FLT_REASON = returnReasonData?.value.map((reason) => reason.value) ?? []
  filters50.value.FLT_INBOUND_TYPE = ReturnInboundTypeData?.value.value ?? '';
...
const returnDetailTableColumnsByReturnId = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'EU040100_GRD05001')
})
...
<template>
  <div class="space-y-3">
    <div class="grid grid-cols-5 gap-3 items-center pl-3 pb-3 border-b">
      <FormGroup
        :label="getComponentData('EU040100_LBL05001', 'caption', 'Brand Channel')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB05001"
          v-model="channelList"
          multiple
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL05002', 'caption', 'Inbound Type')"
        horizontal
      >
        <FormSelect
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL05003', 'caption','Return Reason')"
        horizontal
      >
        <FormSelect
          id="EU040100_CMB05003"
      ...
      <FormGroup
        :label="getComponentData('EU040100_LBL05004', 'caption','Key Word')"
        horizontal
        label-id="OU030100_LBL02001"
        label-width="w-1/3"
      >
        <FormTextInput
          id="EU040200_TXT05001"
          v-model="filters50.FLT_KEY_WORD"
        />
      </FormGroup>
      ...
      >
        {{ getComponentData ('EU040100_BTN05001', 'caption','SEARCH') }}
      </Button>
    </div>

    <div class="pl-3 space-y-2">
      <span>
        {{ getComponentData('EU040100_GRD05001', 'caption','Return Hold by OMS list') }}
      </span>

      <Table
        id="EU040100_GRD05001"
        clickable
