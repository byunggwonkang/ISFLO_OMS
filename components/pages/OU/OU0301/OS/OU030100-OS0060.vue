...
const { userGroupChannelList, filters60 } = storeToRefs(useOrderStatusStore())
...
onMounted(async () => {
  ...
  if (success) {
    filters60.value.FLT_STATUS = [ "10", "20" ];
    filters60.value.FLT_ORDER_TYPES = [ "R" ];
    ...
    filters60.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
    filters60.value.FLT_INCLUDE_REQSUB = "1";
...
const orderDetailTableColumnsByOrderID = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD06002')
})

const orderDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD06003')
})

const tableId = computed(() => {
  return filters60.value.FLT_VIEW_OPT === '0010' ? 'OU030100_GRD06002' : 'OU030100_GRD06003'
})
...
<template>
  <div class="space-y-3">
    ...
    <div class="pl-3 space-y-2">
      ...
      <Table
        v-if="tableId === 'OU030100_GRD06002'"
        id="OU030100_GRD06002"
        clickable
      ...
      <Table
        v-if="tableId === 'OU030100_GRD06003'"
        id="OU030100_GRD06003"
        clickable
        header-variant="secondary"
        class="overflow-x-auto border-b"
