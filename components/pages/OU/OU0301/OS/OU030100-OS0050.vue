...
const { userGroupChannelList, filters50 } = storeToRefs(useOrderStatusStore())
...
onMounted(async () => {
  ...
  if (success) {
    filters50.value.FLT_STATUS = [ "10", "20" ];
    filters50.value.FLT_ORDER_TYPES = [ "E" ];
    filters50.value.FLT_CHANNELS = userGroupChannelList.value.map((channel) => decrypt(channel.USER_CHANNEL_CODE))
    filters50.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
    filters50.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
    filters50.value.FLT_INCLUDE_REQSUB = "0";
    ...
    const { data } = await orderStatus.fetchOrderDetailList50()
...
const onClickOrderDetailRefresh = async () => {
  ...
  filters50.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters50.value.FLT_SPLIT_OPT = orderType?.value.value ?? '';
  filters50.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters50.value.FLT_INCLUDE_REQSUB = "0";
...
const orderDetailTableColumnsByOrderID = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD05002')
})

const orderDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD05003')
})

const tableId = computed(() => {
  return filters50.value.FLT_VIEW_OPT === '0010' ? 'OU030100_GRD05002' : 'OU030100_GRD05003'
})
...
<template>
  <div class="space-y-3">
    <div class="flex space-x-12 items-center pl-3 pb-3 border-b">
      <FormGroup
        :label="getGridData('OU030100_GRD05001', 'caption')"
        horizontal
      >
        <FormSelect
          v-model="channelList"
      ...
      <FormGroup
        :label="getComponentData('OU030100_LBL05001', 'caption')"
        horizontal
        label-width="w-1/3"
        label-id="OU030100_LBL05001"
      >
        <FormSelect
          id="OU030100_CMB05001"
          v-model="viewType"
      ...
      <FormGroup
        :label="getComponentData('OU030100_LBL05002', 'caption')"
        horizontal
        label-id="OU030100_LBL05002"
      >
        <FormSelect
          id="OU030100_CMB05002"
          v-model="orderType"
      ...
      <div class="flex">
        ...
        >
          {{ getComponentData ('OU030100_BTN05001', 'caption') }}
        </Button>
      </div>
    </div>

    <div class="pl-3 space-y-2">
      <span v-if="getGridData('OU030100_GRD05002', 'display') === 'Y'">
        {{ getGridData('OU030100_GRD05002', 'caption') }}
      </span>

      <Table
        v-if="tableId === 'OU030100_GRD05002'"
        id="OU030100_GRD05002"
        clickable
      ...
      <Table
        v-if="tableId === 'OU030100_GRD05003'"
        id="OU030100_GRD05003"
        clickable
        header-variant="secondary"
        class="overflow-x-auto border-b"
