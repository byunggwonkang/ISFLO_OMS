...
const { userGroupChannelList, filters20 } = storeToRefs(useOrderStatusStore())
...
onMounted(async () => {
  ...
  if (success) {
    ...
    const { data } = await orderStatus.fetchOrderDetailList20()
...
const orderDetailTableColumnsByOrderID = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD02002')
})

const orderDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030100_GRD02003')
})

const tableId = computed(() => {
  return filters20.value.FLT_VIEW_OPT === '0010' ? 'OU030100_GRD02002' : 'OU030100_GRD02003'
})
...
<template>
  <div class="space-y-3">
    <div class="flex space-x-12 items-center pl-3 pb-3 border-b">
      <FormGroup
        :label="getGridData('OU030100_GRD02001', 'caption')"
        horizontal
      >
        <FormSelect
          v-model="channelList"
      ...
      <FormGroup
        :label="getComponentData('OU030100_LBL02001', 'caption')"
        horizontal
      ...
      <FormGroup
        :label="getComponentData('OU030100_LBL02002', 'caption')"
        horizontal
        label-id="OU030100_LBL02002"
      >
        <FormSelect
          id="OU030100_LBL02002"
          v-model="orderType"
      ...
      <div class="flex">
        ...
        >
          {{ getComponentData ('OU030100_BTN02001', 'caption') }}
        </Button>
      </div>
    </div>

    <div class="pl-3 space-y-2">
      <span v-if="getGridData('OU030100_GRD02002', 'display') === 'N'">
        {{ getGridData('OU030100_GRD02002', 'caption') }}
      </span>

      <Table
        v-if="tableId === 'OU030100_GRD02002'"
        id="OU030100_GRD02002"
        clickable
      ...
      <Table
        v-if="tableId === 'OU030100_GRD02003'"
        id="OU030100_GRD02003"
        clickable
        header-variant="secondary"
        class="overflow-x-auto border-b"
