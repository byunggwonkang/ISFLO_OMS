...
const { orderStatusList, currentOrderStatus } = storeToRefs(useOrderStatusStore())
...
const tabContentComponentName = computed(() => {
  return `${currentTab.value?.component}OS${decrypt(currentOrderStatus.value?.ORD_STATUS_CODE)}`
})
      ...
      <div class="w-44 flex-none space-y-3 border-r border-gray-200 pt-3 pr-3">
        <div class="flex justify-between items-center">
          <span class="font-black text-md">
            {{ getComponentData('OU030100_BTN00001', 'caption', 'Refresh') }}
          </span>

          <Button
            id="OU030100_BTN00001"
            rounded
            :disabled="loading"
            variant="white"
            border
