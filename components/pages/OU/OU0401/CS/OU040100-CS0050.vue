...
watch(() => transactionType.value, (value) => {
  if (value) {
    informationForm.value.FLT_CS_TRANSACTION_TYPE_CODE = value.value

    switch (value.value) {
      case '0020':
        categoryOptions.value = commonData.getCsCategoryNewReturnOptions
        break
      case '0030':
        categoryOptions.value = commonData.getCsCategoryNewExchangeOptions
        break
      default:
...
watch(() => userRegistered.value, (value) => {
  informationForm.value.FLT_CS_CUST_REGIST_YN = value ? 'Y' : 'N'
})
...
const resetFormData = () => {
  informationForm.value = {
    ...
    FLT_CS_CUST_MOBILE_NO: '',
    FLT_CS_CUST_SID: '',
    FLT_CS_REGIST_DATE: moment().format('YYYY-MM-DD'),
    FLT_CS_REGIST_TIME: moment().format('HH:mm')
  }
...
const onClickDetail = () => {
  switch (informationForm.value.FLT_CS_TRANSACTION_TYPE_CODE) {
    case '0010':
      openOrderDetail()
      break
    default:
      ...
      <div>
        ...
        <div class="grid grid-cols-4 space-x-6 mb-3">
          <FormGroup
            ...
            label-width="w-1/4"
            input-width="w-3/4"
            :label="getComponentData('OU040100_LBL06006', 'caption')"
          >
            <div class="flex items-center gap-x-3">
              <FormSelect
                v-model="transactionType"
                :options="commonData.getTransactionTypeReturnOptions"
              />
              <FormSelect
                v-model="category"
        ...
        <div class="grid grid-cols-4 space-x-6 items-center border-b mb-6 pb-3">
          ...
          <FormGroup
            :label="getComponentData('OU040100_LBL03011', 'caption')"
            label-id="OU040100_LBL03011"
            label-for="OU040100_TXT03006"
            horizontal
          >
            <div class="flex items-center gap-x-3">
              <FormTextInput
                id="OU040100_TXT03006"
                v-model="informationForm.FLT_CS_TRANSACTION_SID"
              />

              <Button
                id="OU040100_BTN03002"
                rounded
                :variant="informationForm.FLT_CS_TRANSACTION_SID !== '' ? 'white' : 'secondary'"
                border
                :disabled="informationForm.FLT_CS_TRANSACTION_SID === ''"
                @click="onClickDetail"
              >
                {{ getComponentData('OU040100_BTN03002', 'caption') }}
              </Button>
