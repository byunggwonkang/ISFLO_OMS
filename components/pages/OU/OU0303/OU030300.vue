...
const { exchangeStatusList, currentExchangeStatus } = storeToRefs(useExchangeStatusStore())

const { loading, toggleLoading } = useLoading()
const { getComponentData } = usePageLoaderComponent([currentTab.value?.component ?? '', 'POP00020'])

watch(currentExchangeStatus, () => {
  exchangeStatus.resetExchangeDetailList()
  // exchangeStatus.resetFilters()
})

onMounted(async () => {
  togglePageLoading()
  isloading.isLoading = true
  await commonData.fetchOU030300CommonData()
  await pageLoader.fetchPageLoadData(currentTab.value?.component ?? '')
  await pageLoader.fetchPageLoadData('POP00300')
  await exchangeStatus.fetchExchangeStatusList()
...
const onClickRefresh = async () => {
  ...
  isloading.isLoading = true
  await Promise.all([
    exchangeStatus.fetchExchangeStatusList(),
    exchangeStatus.fetchExchangeSummaryList(),
  ])
  isloading.isLoading = false
  toggleLoading()
}
...
const tabContentComponentName = computed(() => {
  return `${currentTab.value?.component}ES${decrypt(currentExchangeStatus.value?.EXC_STATUS_CODE)}`
})
      ...
      <div class="w-44 flex-none space-y-3 border-r border-gray-200 pt-3 pr-3">
        <div class="flex justify-between items-center">
          <span class="font-black text-md">
            {{ getComponentData('OU030300_BTN01001', 'caption', 'Refresh') }}
          </span>

          <Button
            id="OU030300_BTN00001"
            rounded
            variant="white"
            border
            size="xs"
        ...
        >
          ...
          <div
            ...
            @click="exchangeStatus.setCurrentExchangeStatus(list)"
          >
            <div
