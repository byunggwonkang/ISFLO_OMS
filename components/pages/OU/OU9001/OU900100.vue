...
const salseReport = useSalesReportsStore()
...
const { filters, userGroupBrandList } = storeToRefs(salseReport)
...
const OrgAmount = ref("0")
const OrgDCAmount = ref("0")
const SalseAmount = ref("0")

const brand = ref({})
const ReportType = ref({})

const fetchReportDailyList = async () => {
  filters.value.FLT_BRAND_CODE = brand.value?.value ?? ''
  filters.value.FLT_REPORT_TYPE = ReportType.value?.value ?? ''

  const { data } = await salseReport.fetchReportDailyList()
...
const onClickExportExcel = async () => {
  ...
  if (filters.value.FLT_REPORT_TYPE === '0010') {
    table = document.getElementById('OU900100_GRD00003')
    fileName = 'Sales Report by Orders_'
  } else {
    table = document.getElementById('OU900100_GRD00004')
    fileName = 'Sales Report by Order+Products_'
  }
...
watch(() => reportList, async (value) => {
  if (value) {
    ...
    for (let i = 0; i < value.value.length; i++) 
    {
      ...
      Org_Amount = Org_Amount + Number(decrypt(value.value[i].ORDER_ITEM_ORG_AMT))
      OrgDC_Amount = OrgDC_Amount + Number(decrypt(value.value[i].ORDER_ITEM_DC_AMT))
      Salse_Amount = Salse_Amount + Number(decrypt(value.value[i].ORDER_ITEM_SALES_AMT))
    }
    ProductQty.value = Product_Qty.toLocaleString()
    OrgAmount.value = Org_Amount.toLocaleString()
    OrgDCAmount.value = OrgDC_Amount.toLocaleString()
    SalseAmount.value = Salse_Amount.toLocaleString()
  }
}, { deep: true })

const tableId = computed(() => {
  return filters.value.FLT_REPORT_TYPE === '0010' ? 'OU900100_GRD00001' : 'OU900100_GRD00002'
})
...
onMounted(async () => {
  ...
  await salseReport.fetchUserGroupBrandList()
  ...
  >
    <template #filters>
      <div class="grid grid-cols-5 gap-6">
        <FormGroup
          :label="getComponentData('OU900100_LBL00001', 'caption','Brand')"
          label-id="OU900100_LBL00001"
          label-for="OU900100_CMB00001"
          horizontal
        >
          <FormSelect
            id="OU900100_CMB00001"
            v-model="brand"
        ...
        <FormGroup
          :label="getComponentData('OU900100_LBL00002', 'caption', 'Order Date')"
          label-id="OU900100_LBL00002"
          horizontal
        >
        ...
        <FormGroup
          :label="getComponentData('OU900100_LBL00003', 'caption','Report Type')"
          label-id="OU900100_LBL00003"
          label-for="OU900100_CMB00002"
          horizontal
        >
          <FormSelect
            id="OU900100_CMB00002"
            v-model="ReportType"
            :options="[
              {
                name: 'By Order',
        ...
        <div>
          <Button
            ...
            @click="onClickSearch"
          >
            {{ getComponentData ('OU900100_BTN00001', 'caption', 'Search') }}
          </Button>
        </div>   
        <div class="text-end">
          <Button
          ...
          > 
            {{ getComponentData ('OU900100_BTN00002', 'caption','EXCEL Export') }}
          </Button>
        </div>     
      </div>
    </template>
    ...
    <Table
      v-if="tableId === 'OU900100_GRD00001'"
      id="OU900100_GRD00001"
      ...
      <TableBody>
        ...
        >                          
          <TableCell class="text-center">
            {{ decrypt(option.ORDER_NO) }}
          </TableCell>
          <TableCell class="text-center">
            {{ decrypt(option.BRAND_SNAME) }}
          </TableCell> 
          <TableCell class="text-center">
            {{ decrypt(option.ORDER_DATE) }}
          </TableCell>  
          <TableCell class="text-center">
          ...
          <TableCell class="text-right">
            {{ Number(decrypt(option.ORDER_ITEM_ORG_AMT)).toLocaleString() }}
          </TableCell>  
          <TableCell class="text-right">
            {{ Number(decrypt(option.ORDER_ITEM_DC_AMT)).toLocaleString() }}
          </TableCell>  
          <TableCell class="text-right">
    ...
    <Table
      v-if="tableId === 'OU900100_GRD00002'"
      id="OU900100_GRD00002"
      class="overflow-x-auto mt-3"
      header-variant="secondary"
    >
      <TableHead>   
      ...
      <TableBody>
        ...
        >                          
          ...
          <TableCell class="text-center">
            {{ option.ITEM_DESCR1 ? decrypt(option.ITEM_DESCR1) : '-' }}
          </TableCell>  
          <TableCell class="text-center">
            {{ option.ITEM_COLOR ? decrypt(option.ITEM_COLOR) : '-' }}
          </TableCell>  
          <TableCell class="text-center">
            {{ option.ITEM_SIZE ? decrypt(option.ITEM_SIZE) : '-' }}
          </TableCell>  
          <TableCell class="text-center">
            {{ option.ITEM_CATEGORY_L ? decrypt(option.ITEM_CATEGORY_L) : '-' }}
          </TableCell>  
          <TableCell class="text-center">
            {{ option.ITEM_CATEGORY_M ? decrypt(option.ITEM_CATEGORY_M) : '-' }}
          </TableCell>  
          <TableCell class="text-center">
            {{ option.ITEM_CATEGORY_S ? decrypt(option.ITEM_CATEGORY_S) : '-' }}
          </TableCell> 
          <TableCell class="text-right">
            {{ Number(decrypt(option.ORDER_ITEM_QTY)).toLocaleString() }}
          </TableCell>  
