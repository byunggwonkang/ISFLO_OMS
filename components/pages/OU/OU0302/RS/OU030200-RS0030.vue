...
const { userGroupChannelList, filters30 } = storeToRefs(useReturnStatusStore())
...
onMounted(async () => {
  ...
  if (success) {
    ...
    filters30.value.FLT_RETURN_TYPES = ["C"];

    const { data } = await returnStatus.fetchReturnDetailList30()
...
const onClickReturnDetailRefresh = async () => {
  filters30.value.FLT_STATUS = returnStatusData?.value.map((status) => status.value) ?? []
  filters30.value.FLT_RETURN_TYPES = ["C"];
  filters30.value.FLT_VIEW_OPT = viewType?.value.value ?? '';
  filters30.value.FLT_CHANNELS = channelList?.value.map((channel) => channel.value) ?? [];
  filters30.value.FLT_DATE_TYPE = dateType?.value.value ?? '';
  filters30.value.FLT_REASON = returnReasonData?.value.map((reason) => reason.value) ?? []
...
const returnDetailTableColumnsByReturnId = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030200_GRD03004')
})

const returnDetailTableColumnsByProducts = computed(() => {
  return pageLoader.getTableData(currentTab.value?.component ?? '' , 'OU030200_GRD03005')
})

const tableId = computed(() => {
  return filters30.value.FLT_VIEW_OPT === '0010' ? 'OU030200_GRD03004' : 'OU030200_GRD03005'
})
...
<template>
  <div class="space-y-3">
    <div class="grid grid-cols-4 gap-3 items-center pl-3 pb-3 border-b">
      ...
      >
        <FormSelect
          ...
          multiple
          :options="commonData.getReturnCancelStatusOptions"
        />
      </FormGroup>
    ...
    <div class="pl-3 space-y-2">
      <span>
        {{ getComponentData('OU030200_LBL03007', 'caption') }}
      </span>

      <Table
        v-if="tableId === 'OU030200_GRD03004'"
        id="OU030200_GRD03004"
        clickable
      ...
      <Table
        v-if="tableId === 'OU030200_GRD03005'"
        id="OU030200_GRD03005"
        clickable
        header-variant="secondary"
        class="overflow-x-auto border-b"
