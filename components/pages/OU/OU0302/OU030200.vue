...
const onClickRefresh = async () => {
  ...
  isloading.isLoading = true
  await Promise.all([
    returnStatus.fetchReturnStatusList(),
    returnStatus.fetchReturnSummaryList(),
  ])
  isloading.isLoading = false
  toggleLoading()
}
...
const tabContentComponentName = computed(() => {
  return `${currentTab.value?.component}RS${decrypt(currentReturnStatus.value?.RET_STATUS_CODE)}`
})
      ...
      <div class="w-44 flex-none space-y-3 border-r border-gray-200 pt-3 pr-3">
        <div class="flex justify-between items-center">
          <span class="font-black text-md">
            {{ getComponentData('OU030200_BTN01001', 'caption', 'Refresh') }}
          </span>

          <Button
            id="OU030200_BTN01001"
            rounded
            variant="white"
            border
            size="xs"
        ...
        >
          ...
          <div
            ...
            @click="returnStatus.setCurrentReturnStatus(list)"
          >
            <div
